...
----------------1---------------------------------
firstToEnd :: (Read a) => [a] -> [a] firstToEnd x  = reverse (x)

----------------2-------------------------------
mindAndMax :: (Ord a) => [a] -> [a] mindAndMax xs = minimum xs : [maximum xs]

----------------3--------------------------------
minorsFirstElement :: (Integral a) => [a] -> [a] minorsFirstElement var = filter (head var >) (tail var)
...
listMulTripletaToList list = l3 (head list) * d3 (head list) * h3(head list) : listMulTripletaToList (tail list)
